...
export async function bubbleSort(data, state) {
  ...
  for (i = 0; i < len; i++) drawUnselect(i);
  ...
  for (i = 0; i < len; i++) {
    for (j = 0; j < len - i - 1; j++) {
      ...
      if (state.pause) while (state.pause) await sleep(100);
...
export async function insertionSort(data, state) {
  ...
  for (i = 1; i < len; i++) {
    ...
    data[j + 1] = key;
    for (k = j + 1; k <= i; k++) drawUpdate(data[k], k);

    for (k = j + 1; k <= i; k++) drawSorted(k);
...
export async function mergeSort(data, state) {
  ...
  for (let i = 0; i < data.length; i++) drawUnselect(i);
...
async function mergeSortRun(data, state) {
  ...
  for (let i = 0; i < data.length; i++) drawAltSelect(state.opt + i);
  ...
  await sleep(state.delay);
  for (let i = 0; i < data.length; i++) drawUnselect(state.opt + i);
...
async function quickSortPartition(data, low, high, state) {
  let i = low,
    j = high,
    pivot_id = Math.floor((low + high) / 2),
    pivot = data[pivot_id];
  drawSelect(pivot_id);
  await sleep(state.delay);
  if (!state.running) return data;
...
export async function heapSort(data, state) {
  ...
  for (let i = 0; i < data.length; i++) drawAltSelect(i);
...
async function heapSortHeapify(data, i, n, state) {
  ...
  if (il < n && data[il] > data[largest]) largest = il;

  if (ir < n && data[ir] > data[largest]) largest = ir;
...
export async function radixLSDSort(data, state) {
  ...
  for (i = 0; i < data.length; i++) drawUnselect(i);
  ...
  for (i = 0; i < k; i++) {
    ...
    d = new Array();
    for (j = 0; j < a.length; j++) d = d.concat(a[j]);
    for (j = 0; j < data.length; j++) data[j] = d[j];

    for (j = 0; j < data.length; j++) {
      drawUpdate(data[j], j);
      if (!state.running) return data;
      if (state.pause) while (state.pause) await sleep(100);
      if (i == k - 1) drawSorted(j);
      await sleep(state.delay);
    }
  }
  return data;
...
function drawSwapSelected(i, j) {
  ...
  first.style.setProperty(
    "height",
    second.style.getPropertyValue("height"),
    null
  );
